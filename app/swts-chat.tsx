...
export function FreeRagChat() {
  const [taskId, setTaskId] = useState<RagTaskId>("free_chat");
  const [sessionId, setSessionId] = useState(() => `free-${crypto.randomUUID()}-all`);
  ...
  return <main className="free-chat-page" dir="rtl"><header className="free-chat-header"><div><p className="eyebrow">جزیره سپید</p><h1>گفت‌وگوی آزاد با سامانه</h1><p>این صفحه خارج از آزمایش است و هیچ پیام یا پاسخی را در پایگاه داده پژوهش ذخیره نمی‌کند.</p></div><Link className="secondary compact-button" href="/">بازگشت به مطالعه</Link></header><section className="free-chat-shell"><div className="free-chat-toolbar"><label>حالت گفتگو<select value={taskId} onChange={(event) => { const next = event.target.value as RagTaskId; setTaskId(next); setSessionId(`free-${crypto.randomUUID()}-${next}`); setMessages([]); setDraft(""); }}><option value="free_chat">گفت‌وگوی آزاد</option>{SWTS_TASK_IDS.map((id) => <option key={id} value={id}>شبیه‌سازی وظیفه: {SWTS_TASKS[id].title}</option>)}</select></label><span><i className={ready ? "ready" : "pending"} />{ready ? "سامانه آماده است" : "در حال اتصال"}</span></div><div className="swts-messages free-messages" aria-live="polite">{!messages.length && <div className="swts-empty"><span>✦</span><strong>گفت‌وگوی تازه</strong><p>در همه حالت‌ها می‌توانید درباره تمام اطلاعات جزیره سپید سؤال کنید.</p></div>}{messages.map((message) => <article key={message.key} className={`swts-message ${message.role}`}><span>{message.role === "user" ? "شما" : "دستیار"}</span>{message.role === "assistant" ? <MarkdownMessage>{message.content}</MarkdownMessage> : <p>{message.content}</p>}</article>)}{busy && <div className="swts-typing"><i /><i /><i /><span>در حال آماده‌کردن پاسخ…</span></div>}<div ref={messagesEnd} /></div>{error && <div className="swts-alert"><p>{error}</p>{!ready && <button onClick={() => void verify()} disabled={busy}>بررسی دوباره</button>}</div>}<form className="swts-composer" onSubmit={(event) => void send(event)}><textarea value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void send(); } }} maxLength={8000} placeholder="پیام خود را بنویسید…" disabled={!ready || busy} /><div><small>این گفتگو فقط تا زمانی که این صفحه باز است نگه داشته می‌شود.</small><button className="chat-send" disabled={!draft.trim() || !ready || busy}>↑</button></div></form></section></main>;
}
